...
: > "$RESULTS_DIR/k6.log"
if docker pull "$K6_IMAGE" >>"$RESULTS_DIR/k6.log" 2>&1; then
  ...
  echo "k6 이미지를 받지 못했다 — 배포는 계속한다" >&2
fi
...
if [ "$K6_STARTED" -eq 1 ]; then
  ...
  wait "$K6_PID"
  K6_EXIT_CODE=$?
  [ "$K6_EXIT_CODE" -eq 0 ] || [ "$K6_EXIT_CODE" -eq 103 ] || K6_STATUS=$K6_EXIT_CODE
  if [ "$K6_EXIT_CODE" -eq 0 ] || { [ "$K6_EXIT_CODE" -eq 103 ] && [ "$CONTROL_STATUS" -eq 0 ]; }; then
    K6_STATUS=0
  fi
fi
set -e
...
echo "k6 판정 코드: $K6_STATUS"
echo "k6 원본 종료 코드: $K6_EXIT_CODE"
echo "k6 제어 종료 코드: $CONTROL_STATUS"
echo "k6 threshold 판정 코드: $THRESHOLD_STATUS"

[ "$K6_STATUS" -eq 0 ] \
  || echo "::warning title=배포 연속성 검사::k6 실행이 정상적으로 끝나지 않았다 (exit=$K6_EXIT_CODE)"
[ "$CONTROL_STATUS" -eq 0 ] \
  || echo "::warning title=배포 연속성 검사::k6 제어 종료에 실패했다"
[ "$THRESHOLD_STATUS" -eq 0 ] \
  || echo "::warning title=배포 연속성 검사::무중단 threshold를 통과하지 못했다"
