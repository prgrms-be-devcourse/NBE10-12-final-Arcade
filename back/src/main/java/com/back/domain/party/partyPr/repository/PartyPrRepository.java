...
public interface PartyPrRepository extends JpaRepository<PartyPr, Long> {
    ...
    @Query("""
            ...
            and (pr.party.owner = :actor or exists (select pm.id from PartyMember pm where pm.party = pr.party and pm.member = :actor and pm.status = 'APPROVED'))
            order by pr.githubUpdatedAt desc
            """)
    List<PartyPr> findReadableByAuthorGithubUserId(@Param("githubUserId") Long githubUserId, @Param("actor") com.back.domain.member.member.entity.Member actor);
