...
export function MyPartyShortcuts({ parties }: { parties: MyParty[] }) {
  ...
  return <section className="section my-party-shortcuts container" data-reveal suppressHydrationWarning>
    <div className="section-head"><h2>내 파티 바로가기</h2><Link className="card-link" href="/mypage">전체 내역 보기 →</Link></div>
    <div className="my-party-row">{activeParties.slice(0, 3).map((party) => <Link className="my-party-card" href={`/party/${party.id}/team`} key={party.id}>
